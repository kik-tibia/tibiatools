<script lang="ts">
  ...
  function writeToUrl() {
    ...
    if (!sa) p.delete("a"); else p.set("a", sa);
    if (!sb) p.delete("b"); else p.set("b", sb);
...
<label>
  ...
  <input type="number" bind:value={a} inputmode="decimal" placeholder="e.g. 12.5" />
</label>

<label>
  <span>Second number</span>
  <input type="number" bind:value={b} inputmode="decimal" placeholder="e.g. 3" />
</label>
</form>
...
<div class="row">
  ...
  <button type="button" on:click={() => { a = ""; b = ""; }}>
    Reset
  </button>
</div>

<style>
  .stack { display: grid; gap: .75rem; max-width: 360px; }
  input { padding: .5rem; font: inherit; }
  .result { margin-top: .75rem; font-size: 1.125rem; }
  .row { display: flex; gap: .5rem; margin-top: .5rem; }
  button { padding: .5rem .75rem; border: 1px solid #bbb; border-radius: .5rem; background: transparent; cursor: pointer; }
</style>
